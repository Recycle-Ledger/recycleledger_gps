package com.example.recycleledger_gps;
...
public class MainActivity extends AppCompatActivity implements AutoPermissionsListener {
    ...
    public void startLocationService() {
        try {
            ...
            if (manager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
                ...
                //위치 요청하기
                manager.requestLocationUpdates(LocationManager.GPS_PROVIDER, minTime, minDistance, (LocationListener) gpsListener);
    ...
    private void showMyLocationMarker(LatLng curPoint) {
        ...
        // 반경추가 10km
        if (circle10KM == null) {
            circle10KM = new CircleOptions().center(curPoint) // 원점
                    .radius(1000)       // 반지름 단위 : m
                    .strokeWidth(1.0f);    // 선너비 0f : 선없음
            //.fillColor(Color.parseColor("#1AFFFFFF")); // 배경색
            circle = map.addCircle(circle10KM);

        } else {
            circle.remove(); // 반경삭제
            circle10KM.center(curPoint);
            circle = map.addCircle(circle10KM);
        }
